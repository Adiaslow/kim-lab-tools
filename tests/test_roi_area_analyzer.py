...
def test_roi_area_result():
    ...
    result = ROIAreaResult(segment_id="s001", region_name="Region1", area_pixels=100)
    assert result.segment_id == "s001"
...
def test_filename_parsing():
    ...
    animal_id, segment_id, region_name = analyzer._parse_filename(
        "M123_s001_Region1.pkl"
    )

    assert animal_id == "M123"
    assert segment_id == "s001"
...
def test_analyze_directory(create_test_files, temp_roi_dir):
    ...
    # Check that segment IDs are correct
    assert set(results_df["segment_id"]) == {"s001", "s002"}
...
def test_get_summary_by_segment(create_test_files, temp_roi_dir):
    """Test segment summary generation."""
    analyzer = ROIAreaAnalyzer(str(temp_roi_dir))
    df = analyzer.analyze_directory()
    summary = analyzer.get_summary_by_segment(df)
