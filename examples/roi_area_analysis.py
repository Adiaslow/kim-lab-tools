...
Example usage:
    python scripts/roi_area_analysis.py /path/to/roi/directory
"""
...
def create_visualizations(df: pd.DataFrame, output_dir: Path) -> None:
    ...
    with tqdm(total=2, desc="Creating visualizations") as pbar:
        ...
        # 2. Area trends across segments
        plt.figure(figsize=(12, 6))
        segment_means = (
            df.groupby(["segment_id", "region_name"])["area_pixels"].mean().unstack()
        )
        segment_means.plot(marker="o")
        plt.title("ROI Area Trends Across Segments")
...
def save_results(
    ...
    Args:
        ...
        segment_summary: DataFrame containing summary by segment
...
def print_statistics(results_df: pd.DataFrame) -> None:
    ...
    print(f"Number of segments: {results_df['segment_id'].nunique()}")
    print("\nMean area by region:")
    ...
    try:
        ...
        if input_path.is_file():
            results_df = results_df[
                results_df.apply(
                    lambda x: f"{x['animal_id']}_s{x['segment_id']}_{x['region_name']}.pkl"
                    == input_path.name,
                    axis=1,
                )
            ]
        ...
        segment_summary = analyzer.get_summary_by_segment(results_df)

        # Save results
        save_results(results_df, region_summary, segment_summary, output_dir)
